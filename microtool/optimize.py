...
# A LossFunction takes an N×M Jacobian matrix, an array with M parameter scales, an array with M parameter uses, and the
# noise variance. It should return a scalar loss.
...
def crlb_loss(jac: np.ndarray, scales: Sequence[float], uses: Sequence[bool], noise_var: float) -> float:
    """
    ...
    :param uses: A length-M boolean array specifying which parameters should be included in the loss.
    :param noise_var: Noise variance.
    ...
    return np.linalg.eigvalsh(information)[uses].sum()
