...
def brute_wrapper(fun: callable, x0: np.ndarray, args=(), Ns: int = 10,
...
def brute_force(fun: callable, x0: np.ndarray, args=(), Ns: int = 10,
                bounds: List[Tuple[Optional[float], Optional[float]]] = None, constraints=None,
                **options) -> OptimizeResult:
    ...
    if options["show_plot"]:
        if Nx != 2:
            raise ValueError("Cant visualize loss function with more than 2 parameters")
        fig = plt.figure()
        ax = fig.add_subplot(projection='3d')
        x1 = grid[:,0]
        x2 = grid[:,1]

        ax.plot_trisurf(x1,x2,loss)
        plt.show()
