...
from .bval_delta_pulse_relations import constrained_dependencies
from .gradient_sampling.utils import unitvector_to_angles, angles_to_unitvectors
from .gradient_sampling import sample_uniform_half_sphere
from .scanner_parameters import ScannerParameters, default_scanner
from .utils.math import is_smaller_than_with_tolerance, is_higher_than_with_tolerance, random_uniform
...
from .pulse_relations import b_value_from_diffusion_pulse, pulse_magnitude_from_b_value, diffusion_pulse_from_echo_time
...
class AcquisitionParameters:
    ...
    def __init__(self,
        ...
        self.values = values.astype(float, copy=True).ravel()  # Makes a copy and ensures it is a 1D float array.
        self.unit = unit
        self.scale = scale
        self.symbol = symbol
...
class AcquisitionScheme(Dict[str, AcquisitionParameters], ABC):
    ...
    @property
    def scaled_free_parameter_vector(self) -> np.ndarray:
        """
        Returns all the scaled free parameters as a flattened vector.
    ...
    def set_scaled_free_parameter_vector(self, vector: np.ndarray) -> None:
        """
        A setter function for the scaled free parameters.

        :param vector: The parameter vector you wish to assign to the scheme
        """
        # Note: Since parameter.free_values and parameter.scale both have the same unit, the parameter vector is
        # dimensionless.
        vector = vector.ravel()
        if self.scaled_free_parameter_vector.size != vector.size:
            raise ValueError(
        ...
        for key in self.free_parameter_keys:
            ...
            self[key].free_values = vector[i:(i + stride)] * self[key].scale
    ...
    def get_parameter_from_scaled_parameter_vector(self, parameter: str, x: np.ndarray) -> np.ndarray:
        """
    ...
    def _copy_and_update_scaled_parameter(self, parameter: str, x: np.ndarray):
        """
        ...
        update_values = self.get_parameter_from_scaled_parameter_vector(parameter, x)
...
class DiffusionAcquisitionScheme(AcquisitionScheme):
    ...
    :param pulse_magnitudes: The gradient magnitudes in tesla per meter or equivalently mT/mm
    :param gradient_directions: A list or numpy array of direction cosines.
    :param pulse_durations: A list or numpy array of pulse durations δ in seconds.
    ...
    def __init__(self,
                 pulse_magnitudes: Union[List[float], np.ndarray],  # [mT/mm]
                 gradient_directions: Union[List[Tuple[float, float, float]], np.ndarray],  # [-]
                 pulse_durations: Union[List[float], np.ndarray],  # [s]
        ...
        # TODO: Check constraints here or in b_value_from_diffusion_pulse.
        b_values = b_value_from_diffusion_pulse(pulse_durations, pulse_intervals, pulse_magnitudes,
        ...
        super().__init__({
            # TODO: Change DiffusionPulseMagnitude scale to 1000 or 10000?
            'DiffusionPulseMagnitude': AcquisitionParameters(
                values=pulse_magnitudes, unit=GRADIENT_UNIT, scale=1. * unit('mT/mm'), symbol="|G|", lower_bound=0.0,
                upper_bound=5e3 * unit('mT/mm')
            ),
            ...
            'DiffusionPulseDuration': AcquisitionParameters(
                values=pulse_durations, unit=PULSE_TIMING_UNIT, scale=PULSE_TIMING_SCALE, symbol=r"$\delta$",
                fixed=False,
    ...
    @classmethod
    def from_bvals(cls, b_values: np.ndarray, b_vectors: np.ndarray, pulse_durations: np.ndarray,
        ...
        :param pulse_intervals: A list or numpy array of pulse intervals Δ in seconds.
        :param echo_times: A list or numpy array of the echo times in seconds.
        :param scanner_parameters: A ScannerParameters object containing the scanner hardware properties.
        :raise ValueError: b-vectors are not unit vectors or lists have unequal length.
        :return: DiffusionAcquisitionScheme
        """

        # Convert b-values to pulse magnitudes.
        # TODO: Check constraints here or in pulse_magnitude_from_b_value.
        pulse_magnitude = pulse_magnitude_from_b_value(b_values, pulse_durations, pulse_intervals, scanner_parameters)

        return cls(pulse_magnitude, b_vectors, pulse_durations, pulse_intervals, echo_times, scanner_parameters)
    ...
    def _check_constraints(self):
        for desc, constraint in self.constraints.items():
            fun_val = constraint.fun(self.scaled_free_parameter_vector)
    ...
    def b_values(self) -> np.ndarray:
        ...
        """
        return b_value_from_diffusion_pulse(self.pulse_durations, self.pulse_intervals, self.pulse_magnitude,
                                            self.scanner_parameters)
    ...
    # TODO: make consistent use of singular/plural forms
    @property
    def pulse_durations(self) -> np.ndarray:
        """
        An array of N pulse durations in seconds.
        """
        return self['DiffusionPulseDuration'].values
    ...
    def constraints(self) -> Dict[str, NonlinearConstraint]:
        ...
        if not (self['DiffusionPulseDuration'].fixed and self['DiffusionPulseInterval'].fixed):
            ...
            def delta_constraint_fun(x: np.ndarray):
                ...
                pulse_duration = self._copy_and_update_scaled_parameter('DiffusionPulseDuration', x)
                pulse_interval = self._copy_and_update_scaled_parameter('DiffusionPulseInterval', x)
                t_rise = self.scanner_parameters.t_rise_max
                t_180 = self.scanner_parameters.t_180
                return (pulse_interval - (pulse_duration + t_rise + t_180)) / self['DiffusionPulseInterval'].scale
            ...
            constraints["PulseIntervalLargerThanPulseDuration"] = delta_constraint

        if not self['DiffusionPulseMagnitude'].fixed:
            def gradient_constraint_fun(x: np.ndarray):
                # TODO: What is the purpose of updating the parameters here?
                g = self._copy_and_update_scaled_parameter('DiffusionPulseMagnitude', x)
                g_max = self.scanner_parameters.g_max
                return (g_max - g) / self['DiffusionPulseMagnitude'].scale
        ...
        if not (self['DiffusionPulseDuration'].fixed and
                self['DiffusionPulseInterval'].fixed and
                self['EchoTime'].fixed):
            def echo_constraint_fun(x: np.ndarray):
                # TODO: What is the purpose of updating the parameters here?
                self.set_scaled_free_parameter_vector(x)
                echo_time = self._copy_and_update_scaled_parameter("EchoTime", x)
                t_min = minimal_echo_time(self.b_values, self.scanner_parameters)

                return (echo_time - t_min) / self['EchoTime'].scale
    ...
    def fix_b0_measurements(self) -> None:
        ...
        for par in ["DiffusionPulseMagnitude", "DiffusionPulseDuration", "DiffusionPulseInterval", "EchoTime"]:
...
class DiffusionAcquisitionSchemeDelta(AcquisitionScheme):
    """
    Defines a diffusion MR acquisition scheme.
    ...
    _required_parameters = ['pulse_magnitudes', 'gradient_directions', 'pulse_durations', 
                            'pulse_intervals', 'echo_times']

    def __init__(self,
                 pulse_magnitudes: Union[List[float], np.ndarray],
                 gradient_directions: Union[List[Tuple[float, float, float]], np.ndarray],
                 pulse_durations: Union[List[float], np.ndarray],
        ...
        if echo_times is None:
            ...
            echo_times = random_uniform(min_echo_time, MAX_TE, size=len(b_values))
        ...
        super().__init__({
            'DiffusionPulseMagnitude': AcquisitionParameters(
                values=pulse_magnitudes, unit=GRADIENT_UNIT, scale=1., symbol="|G|", 
    ...
    def random_shell_initialization(cls, n_shells: int, n_directions: int, model_dependencies: list,
        ...
        return cls(pulse_magnitudes=random_scheme['pulse_magnitudes'],
                   gradient_directions=gradient_directions,
                   pulse_durations=random_scheme['pulse_durations'],
    ...
    def b_values(self) -> np.ndarray:
        ...
        b_values = b_value_from_diffusion_pulse(self.pulse_durations, self.pulse_intervals, self.pulse_magnitude,
    ...
    def pulse_durations(self) -> np.ndarray:
        ...
        An array of N pulse durations in seconds.
        """
        pulse_durations = self['DiffusionPulseDuration'].values
    ...
    def constraints(self) -> Dict[str, NonlinearConstraint]:
        ...
        def delta_constraint_fun(x: np.ndarray):
            ...
            pulse_durations = self._copy_and_update_scaled_parameter('DiffusionPulseDuration', x)
            pulse_intervals = self._copy_and_update_scaled_parameter('DiffusionPulseInterval', x)
            
            # TODO: t_rise depends on the gradient magnitude.
            t_rise = self.scanner_parameters.t_rise_max
            t_180 = self.scanner_parameters.t_180
            
            return (pulse_intervals - (pulse_durations + t_rise + t_180)) / self['DiffusionPulseInterval'].scale
        ...
        def gradient_b_val_constraint_fun(x: np.ndarray):
            ...
            pulse_durations = self._copy_and_update_scaled_parameter('DiffusionPulseDuration', x)
            pulse_intervals = self._copy_and_update_scaled_parameter('DiffusionPulseInterval', x)               
            b_val = b_value_from_diffusion_pulse(pulse_durations, pulse_intervals, g, self.scanner_parameters)

            return (B_MAX - b_val) / (1 * unit('s/mm²'))
...
class DiffusionAcquisitionSchemeBValue(AcquisitionScheme):
    """
    Defines a diffusion MR acquisition scheme.
    ...
    # Cristina 21-06: removed pulse duration and interval from initialization: they will be computed from TE and b-val
    # relation
    def __init__(self,
                 gradient_directions: Union[List[Tuple[float, float, float]], np.ndarray],
    ...
    def pulse_durations(self) -> np.ndarray:
        ...
        An array of N pulse durations in seconds.
        """
        pulse_duration, _, _ = diffusion_pulse_from_echo_time(self.echo_times, self.scanner_parameters)
    ...
    def pulse_intervals(self) -> np.ndarray:
        ...
        An array of N pulse intervals in seconds.
        """
        _, pulse_interval, _ = diffusion_pulse_from_echo_time(self.echo_times, self.scanner_parameters)
    ...
    def constraints(self) -> Dict[str, NonlinearConstraint]:
        ...
        def bval_te_constraint_fun(x: np.ndarray):
            ...
            pulse_duration, pulse_interval, pulse_magnitude = diffusion_pulse_from_echo_time(
                echo_times, self.scanner_parameters)

            # This computation is based on maximal G
            b_from_te = b_value_from_diffusion_pulse(pulse_duration, pulse_interval, pulse_magnitude,
                                                     self.scanner_parameters)

            return (b_values - b_from_te) / self['B-Values'].scale
...
class InversionRecoveryAcquisitionScheme(AcquisitionScheme):
    ...
    def constraints(self) -> Dict[str, ConstraintTypes]:
        ...
        def time_constraint_fun(x: np.ndarray):
            ...
            ti = self._copy_and_update_scaled_parameter('InversionTime', x)
            te = self._copy_and_update_scaled_parameter('EchoTime', x)
            tr = self._copy_and_update_scaled_parameter('RepetitionTimeExcitation', x)
            return (tr - te - ti) / self['EchoTime'].scale
...
class ReducedDiffusionScheme(AcquisitionScheme):
    ...
    def constraints(self) -> Dict[str, NonlinearConstraint]:
        def fun(x: np.ndarray) -> np.ndarray:
            ...
            return (te - te_min) / self['EchoTime'].scale

        return {'Minimum TE': NonlinearConstraint(fun, 0.0, np.inf)}


# TODO: Revise this function. The lower bound on random echo time depends on the b-value (or pulse duration/interval)
#  and the lower bound on the pulse interval depends on t_180 and the rise time.
def random_parameter_definition(required_params: List, randomization_constraints: List, n_shells: int,
    ...
    for param in required_params:
        ...
        if param == 'gradient_directions':
            ...
            scheme_params[param] = random_uniform(bounds[0], bounds[1], size=n_shells)
    ...
    while not np.all(constrained_dependencies(randomization_constraints, scheme_params, scanner_parameters)):
        ...
        if 'DiffusionPulseDuration' and 'DiffusionPulseInterval' not in randomization_constraints:
            scheme_params['b_values'][mask] = random_uniform(B_VAL_LB, B_VAL_UB, size=np.sum(mask))
            scheme_params['echo_times'][mask] = random_uniform(New_minimal_echo_time(default_scanner), MAX_TE, 
                                                                  size=np.sum(mask))
            
        elif 'DiffusionPulseDuration' and 'DiffusionPulseInterval' in randomization_constraints:
            
            # scheme_params['pulse_magnitudes'][mask] = random_uniform(0, default_scanner.g_max,
            #                                                     size=np.sum(mask))
            scheme_params['echo_times'][mask] = random_uniform(New_minimal_echo_time(default_scanner), MAX_TE,
                                                                  size=np.sum(mask))
            scheme_params['pulse_durations'][mask] = random_uniform(PULSE_TIMING_LB, PULSE_TIMING_UB,
                                                                       size=np.sum(mask))
            scheme_params['pulse_intervals'][mask] = random_uniform(PULSE_TIMING_LB, PULSE_TIMING_UB,
                                                                       size=np.sum(mask))
    
            scheme_params['pulse_magnitudes'][mask] = np.sqrt(
                B_VAL_UB / (
                    GAMMA**2 * (
                        scheme_params['pulse_durations'][mask]**2 *
                        (scheme_params['pulse_intervals'][mask] - scheme_params['pulse_durations'][mask] / 3) +
                        (scanner_parameters.t_rise_max**3) / 30 -
                        (scheme_params['pulse_durations'][mask] * scanner_parameters.t_rise_max**2) / 6)
                )
            )
...
def param_initialization_bounds(parameter: str):
    ...
    parameter_bound_dict = {
        ...
        'pulse_durations': [PULSE_TIMING_LB, PULSE_TIMING_UB],
        'pulse_magnitudes': [0 * unit('mT/mm'), 265e-3 * unit('mT/mm')]
        }
